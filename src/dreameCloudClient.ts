...
export class DreameCloudClient {
    ...
    async login(): Promise<void> {
        const tryRegions: Country[] = this.region === 'cn' ? ['cn'] : [this.region, 'cn'];
    ...
    async start(did: string) {
        return this.sendCommand(did, 'start_clean');
    }

    async pause(did: string) {
        return this.sendCommand(did, 'pause');
    }

    async dock(did: string) {
        return this.sendCommand(did, 'return_dock');
    }
